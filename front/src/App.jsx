...
export default function TreeGenApp() {
    ...
    function addParticipantToProject() {
        ...
        if (!id) return;
        setProjects(prev => prev.map(p => (p.id === participantsModal.projectId ? { ...p, participants: [...p.participants, id] } : p)));
        setNewParticipantId('');
    }

    function removeParticipant(pid, uid) {
        setProjects(prev => prev.map(p => (p.id === pid ? { ...p, participants: p.participants.filter(x => x !== uid) } : p)));
    }
            ...
            <div className="max-w-6xl mx-auto shadow-lg rounded-lg overflow-hidden bg-white">
                ...
                <div className="flex">
                    ...
                    <div className="w-[12%] border-r border-green-100 p-4">
                        <button className="w-full text-left px-3 py-2 rounded hover:bg-green-50 font-medium text-green-700" onClick={() => alert('Go to Projects page')}>
                            Projects
                        </button>
                    </div>
                    ...
                    <div className="w-[88%] p-4 bg-green-50">
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-3">
                                ...
                                <select value={numberSort} onChange={e => setNumberSort(e.target.value)} className="border rounded px-2 py-1">
                                ...
                                <input className="w-20 border rounded px-2 py-1" placeholder="from" value={numRange.from} onChange={e => setNumRange(r => ({ ...r, from: e.target.value }))} />
                                <input className="w-20 border rounded px-2 py-1" placeholder="to" value={numRange.to} onChange={e => setNumRange(r => ({ ...r, to: e.target.value }))} />

                                <label className="text-sm text-slate-600">Name filter:</label>
                                <input className="border rounded px-2 py-1" placeholder="letters..." value={nameFilter} onChange={e => setNameFilter(e.target.value)} />
                            </div>

                            <div>
                                <button className="px-3 py-1 border rounded text-green-700" onClick={() => { setNumberSort('none'); setNumRange({ from: '', to: '' }); setNameFilter(''); setQuery(''); }}>
                                    Clear
                                </button>
                            </div>
                        </div>
                        ...
                        {/* Table header */}
                        <div className="grid grid-cols-[80px_1fr_40px] items-center text-sm font-medium text-slate-600 border-b pb-2">
                            <div>#</div>
                              ...
                              border-b border-green-200 hover:bg-green-100/60 transition cursor-move relative`}
                                >
                                    ...
                                    <div className="px-2 flex items-center justify-between">
                                        <div>
                                            {editingFor === p.id ? (
                                                <input value={editingNameValue} onChange={e => setEditingNameValue(e.target.value)} className="border px-2 py-1 rounded w-64" />
                                            ) : (
                                    ...
                                    <div className="px-2 text-right relative">
                                        <button onClick={() => setMenuOpenFor(menuOpenFor === p.id ? null : p.id)} className="px-2 py-1 rounded hover:bg-green-100">
                                            ⋯
                                        </button>

                                        {menuOpenFor === p.id && (
                                            <div className={`absolute right-0 w-40 bg-white border rounded shadow-md z-20 ${shouldOpenUpwards(idx)
                                                    ? 'bottom-full mb-1'
                                                    : 'top-full mt-1'
                                                }`}>
                                                <button onClick={() => openProject(p.id)} className="w-full text-left px-3 py-2 hover:bg-green-50">Open</button>
                                                <button onClick={() => openEditName(p.id)} className="w-full text-left px-3 py-2 hover:bg-green-50">Edit name</button>
                                                <button onClick={() => openParticipants(p.id)} className="w-full text-left px-3 py-2 hover:bg-green-50">Add participants</button>
                                                <button onClick={() => deleteProject(p.id)} className="w-full text-left px-3 py-2 text-red-600 hover:bg-red-50">Delete</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        {editingFor && (
                            <div className="mt-3 flex gap-2">
                                <button onClick={saveEditName} className="px-3 py-1 bg-green-600 text-white rounded">Save</button>
                                <button onClick={() => { setEditingFor(null); setEditingNameValue(''); }} className="px-3 py-1 border rounded">Cancel</button>
                            </div>
                        )}

                        {/* Add + button */}
                        <div className="mt-4">
                            <button onClick={addProject} className="px-3 py-2 bg-green-700 text-white rounded">+ Add project</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            {participantsModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative bg-white rounded-lg shadow-lg w-96 p-6 z-60">
                        <h3 className="text-lg font-medium text-green-700">Project {participantsModal.projectId} — Participants</h3>
                        <div className="mt-4">
                            <ul className="space-y-2 max-h-60 overflow-y-auto">
                                {getParticipants().map(uid => (
                                    <li key={uid} className="flex items-center justify-between border rounded px-3 py-2">
                                        <div>{uid}</div>
                                        <button onClick={() => removeParticipant(participantsModal.projectId, uid)} className="text-red-600 text-sm">Remove</button>
                                    </li>
                                ))}
                            </ul>
                            ...
                            <div className="mt-4 flex gap-2">
                                ...
                                />
                                <button onClick={addParticipantToProject} className="px-3 py-1 bg-green-600 text-white rounded">Add</button>
                            </div>

                            <div className="mt-4 text-right">
                                <button onClick={() => setParticipantsModal({ open: false, projectId: null })} className="px-3 py-1 border rounded">Close</button>
                            </div>
                        </div>
                    </div>
                </div>
